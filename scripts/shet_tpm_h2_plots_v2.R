...
method_h2 <- Sys.getenv("METHOD_H2", "all_snps_tmm_raw")
...
analysis_root <- file.path(runs_dir, "_analysis", paste0("shet_", method_h2, "_v2"))
dir.create(file.path(analysis_root, "plots"), recursive = TRUE, showWarnings = FALSE)
...
if (nrow(dt) == 0L) stop("No overlapping genes between h2 summary and Shet table.")

epsilon <- min(dt$h2_GREML, na.rm = TRUE)
dt_ex <- dt %>% filter(h2_GREML > epsilon)
if (nrow(dt_ex) == 0L) stop("No rows left after epsilon filter (h2_GREML > min(h2_GREML)).")

# PI-style summary: by existing Shet deciles
d_summ <- dt_ex %>%
  group_by(decile) %>%
  summarise(
...
# Save tables
# --------------------------------------------------
fwrite(as.data.table(dt), file.path(analysis_root, "tables", "joined_h2_shet_all.tsv"), sep = "\t")
fwrite(as.data.table(dt_ex), file.path(analysis_root, "tables", "joined_h2_shet_excluding_epsilon.tsv"), sep = "\t")
fwrite(as.data.table(d_summ), file.path(analysis_root, "tables", "decile_summary_mean_se.tsv"), sep = "\t")
...
fwrite(as.data.table(audit), file.path(analysis_root, "tables", "run_audit.tsv"), sep = "\t")

# --------------------------------------------------
# Plots (GWAS intentionally omitted)
# --------------------------------------------------
p_decile <- ggplot(d_summ, aes(x = decile, y = mean_h2)) +
  ...
  labs(
    title = "Mean h2 by Shet decile (epsilon-filtered)",
...
ggsave(file.path(analysis_root, "plots", "plot1_h2_mean_se_by_decile.png"), p_decile, width = 7.5, height = 4.8, dpi = 300)
...
p_post <- ggplot(d_summ, aes(x = mean_post, y = mean_h2)) +
  ...
  geom_line(color = "steelblue", linewidth = 0.8) +
  labs(
    title = "Mean h2 by mean Shet posterior mean (epsilon-filtered)",
...
ggsave(file.path(analysis_root, "plots", "plot2_h2_mean_se_by_mean_post.png"), p_post, width = 7.5, height = 4.8, dpi = 300)

message("Saved v2 Shet h2 analysis to: ", analysis_root)
